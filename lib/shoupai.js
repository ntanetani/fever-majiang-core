...
module.exports = class Shoupai {
    ...
    static valid_pai(p) {
        // 基本の牌パターンチェック
        if (!p.match(/^(?:[mps]\d|z[1-7])_?\*?[\+\=\-]?$/)) return null;

        // 三人麻雀では萬子の2-8は使用しない
        const s = p[0], n = +p[1];
        if (s === 'm' && n >= 2 && n <= 8) return null;
    ...
    get_peng_mianzi(p) {
        ...
        let mianzi = [];
        let s = p[0], n = + p[1] || 5, d = p.match(/[\+\=\-]$/);
        // 方向指定がない場合は例外ではなく不成立扱いにする
        if (!d) return mianzi;
        if (this._lizhi) return mianzi;

        // 三人麻雀では萬子の2-8は使用しないため、これらの場合は空の配列を返す
        if (s == 'm' && n >= 2 && n <= 8) return mianzi;
        // 三人麻雀では字牌の6,7（白、發）は使用しない
